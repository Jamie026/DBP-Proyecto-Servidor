...
def get_groups_by_user_id(user_id):
    roles = Rol.query.filter_by(user_id=user_id).all()
    groups, members, admins = [], [], []
    for rol in roles:
        group = Group.query.get(rol.group_id)
        groups.append(group.get_data())
        admins.append(rol.admin)
        members.append(Rol.query.filter_by(group_id=group.id).count())
...
def check_member(group_id, user_id):
    admin = Rol.query.filter_by(user_id=user_id, group_id=group_id, admin=True).first()
    if not admin:
...
def get_members(group_id):
    roles = Rol.query.filter_by(group_id=group_id).all()
    members, admins = [], []
    for rol in roles:
...
def create_group():
    ...
    if not validateData(data, ["name", "password", "date", "user"]):
    ...
    db.session.commit()
    create_member(user, new_group, True)
    return {"message": "Grupo creado", "id": new_group.id}, 200

@groups.route("/joinGroup", methods=["POST"])
def join_group():
    data = request.get_json()
    if not validateData(data, ["id", "password", "user"]):
        return {"error": "Datos incorrectos"}, 403
    group_id, password, user_id = data.get("id"), data.get("password"), data.get("user")
    ...
    new_member = create_member(user, group, False)
    if new_member:
        members = Rol.query.filter_by(group_id=group_id).count()
...
def delete_member(group_id, user_id):
    members = Rol.query.filter_by(group_id=group_id).all()
    if len(members) == 1:
        ...
        db.session.delete(Rol.query.filter_by(group_id=group_id, user_id=user_id))
    db.session.commit()
    return {"message": "Salida del grupo exitosa"}, 200

def create_member(user, group, admin):
    member = Rol.query.filter_by(user_id=user.id, group_id=group.id).first()
    if member is not None:
        return False
    new_rol = Rol(user, group, admin)
